...
const { publishScanPlays } = require('./server/actions-publisher');
const { startGraderCron, gradePendingPlays } = require('./server/actions-grader');

// Wrap runMorningScan so every successful scan auto-publishes plays.
...
// Start the result grader (every 30 min, MLB only for now)
startGraderCron();
...
app.post('/ai-brief', (req, res) => {
  ...
  // Current models (Apr 2026): claude-opus-4-7, claude-opus-4-6,
...
// ─── /api/admin/grade-now (manual grader trigger) ───
